...
function verifyTasksChange(
  expectedStat = {CREATED: 0, COMPLETED: 0, CANCELLED: 0, ALL: 0}
) {
  ...
  let task1ID = -1,
    ...
    tests = [
      ...
      },
      {
        ...
        query: () =>
          `mutation {edit(id:"${task1ID}", description:"helloworld")}`,
        test: async res => {
      ...
      },
      {
        name: "fetch tasks",
        query: () =>
          `query{tasks(filter:"ALL", timestamp:"${new Date().valueOf()}"){id, description, status, created, updated}}`,
        test: async res => {
          ...
          await Promise.all(
            template.map(async ({task, id, status}) => {
              await Promise.all(
                fields.map(field => task.should.have.property(field))
              );
...
function apolloAPI() {
  ...
  tests.forEach(({name, mutation, variables, test}) =>
    it(name, async () => {
      if (variables instanceof Function) variables = variables();
      try {
...
describe("start server", () => {
  it("hello test world", () => {
    return new Promise(resolve => setTimeout(resolve, 1000, "setup complete"));
  });
});
