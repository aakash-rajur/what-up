...
class Postgres {
  ...
  async addUser(hash) {
    this.checkConnection();
    return formatQuery(
      await this.pool.query(`select * from add_user('${hash}')`)
    );
  }
  ...
  async deleteUser(hash) {
    ...
    return formatQuery(
      await this.pool.query(`select * from delete_user('${hash}')`)
    );
  }
  ...
  async addTask(hash, description) {
    ...
    return formatQuery(
      await this.pool.query(
        `select * from add_task('${hash}', '${description}')`
      )
    );
  }
  ...
  async updateTask(taskId, newStatus) {
    ...
    return formatQuery(
      await this.pool.query(
        `select * from update_task('${taskId}','${newStatus}')`
      )
    );
  }
  ...
  async editTask(taskId, description) {
    ...
    return formatQuery(
      await this.pool.query(
        `select * from edit_task('${taskId}', '${description}')`
      )
    );
  }
  ...
  async deleteTasks(user) {
    this.checkConnection();
    return formatQuery(
      await this.pool.query(`select * from delete_tasks('${user}')`)
    );
  }
  ...
  async getTasks(hash, status) {
    this.checkConnection();
    return formatQuery(
      await this.pool.query(`select * from get_tasks('${hash}', '${status}')`),
      false
    );
  }
  ...
  async updateAllTasks(hash, selected, status) {
    ...
    return formatQuery(
      await this.pool.query(
        `select * from update_all_tasks('${hash}','${selected}','${status}')`
      )
    );
  }
  ...
  async getStats(hash) {
    this.checkConnection();
    return formatQuery(
      await this.pool.query(`select * from get_stats('${hash}')`),
      false
    );
  }
  ...
  async doesUserExist(hash) {
    this.checkConnection();
    return formatQuery(
      await this.pool.query(`select * from does_user_exist('${hash}')`)
    );
  }
