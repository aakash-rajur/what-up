...
export function promiseSetState(context) {
  return newState =>
    new Promise(resolve => context.setState(newState, resolve));
}
...
export function wait(delay, cb, ...args) {
  let timeout = null,
    promise = new Promise(resolve => {
      timeout = setTimeout(
        async () => resolve(cb && (await cb(...(args || [])))),
        delay
      );
    });
...
export function getFormattedTimestamp(timestamp) {
  ...
  return `${padStart(hr)}: ${padStart(date.getMinutes())} ${suffix}, ${
    DAY_MAP[date.getDay()]
  } ${date.getDate()} ${MONTH_MAP[date.getMonth()]}`;
}
...
export function parseNotification(cb) {
  return ({data}) => {
    const {
      [ON_NOTIFICATION]: {action, data: payload, timestamp}
    } = data;
    return cb({
      action,
      timestamp,
