...
export class App extends Component {
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        <TaskList filter={filter} timestamp={timestamp} action={action} />
        <Footer />
        <SnackBar
          timeout={2000}
  ...
  getNextUpdateALlStatus() {
    if (!this.props || !this.props.stats) return TASK_CREATED;
    const {[TASK_ALL]: total, [TASK_COMPLETED]: completed} = this.props.stat,
      {filter} = this.state;
    if (filter === TASK_ALL)
